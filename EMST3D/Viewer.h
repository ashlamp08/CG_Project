...
class Viewer : public QGLViewer, QOpenGLFunctions_2_1 {
  ...
  public:
	Viewer(QWidget *parent)
		: QGLViewer(CGAL::Qt::createOpenGLContext(), parent), m_showAxis(false), m_showMST(false), m_showVertex(true), m_showDEdge(true), m_showVEdge(false), m_showFacet(false), m_isFlat(false), m_fRadius(1.), m_curMode(NONE), m_isPress(false), m_isMoving(false), m_hasNewPt(false), m_selMode(NORMAL), m_nearestNb(NULL), m_hasEmptyS(false), m_showTrackball(true) {
		...
		transfo1_MST= new std::vector<float>();
		transfo2_MST = new std::vector<float>();
		transfo3_MST = new std::vector<float>();
		transfo4_MST = new std::vector<float>();
  ...
  public Q_SLOTS:
	...
	inline void toggleShowMST(bool flag){
		m_showMST = flag;
		updateGL();
	}
	inline void toggleShowVertex(bool flag) {
		m_showVertex = flag;
		updateGL();
	}
	inline void toggleShowDEdge(bool flag) {
		m_showDEdge = flag;
		updateGL();
	}
	inline void toggleShowVEdge(bool flag) {
		m_showVEdge = flag;
		updateGL();
	}
	inline void toggleShowFacet(bool flag) {
		m_showFacet = flag;
		updateGL();
	}
	inline void toggleFlat(bool flag) {
		m_isFlat = flag;
		updateGL();
	}
	/*inline void setSceneRadius(float radius){
  ...
  private:
	...
	QColor color;
	static const int vaoSize = 30;
	static const int vboSize = 38;
	// define material
	QVector4D ambient;
	QVector4D diffuse;
	QVector4D specular;
	...
	std::vector<float> *transfo1_MST;
	std::vector<float> *transfo2_MST;
	std::vector<float> *transfo3_MST;
	std::vector<float> *transfo4_MST;
