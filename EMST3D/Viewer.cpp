...
void Viewer::init() {
	...
	// scene are defined by a sphere of 2.0, camera at thcame center, i.e. (0, 0, 0)
...
void Viewer::compute_elements() {
	...
	transfo1_MST->resize(0);
	transfo2_MST->resize(0);
	transfo3_MST->resize(0);
	transfo4_MST->resize(0);
	...
	//for (Segment_3 seg : m_pScene->m_mst) {
	...
	//Draw MST Edges
	for (Segment_3 seg: m_pScene->m_mst) {
		drawEdge(seg.vertex(0), seg.vertex(1), pos_MST);
		...
		// stock the transformation
		for (int i = 0; i < 4; i++)
			transfo1_MST->push_back((float)matrix.data()[i]);
		for (int i = 4; i < 8; i++)
			transfo2_MST->push_back((float)matrix.data()[i]);
		for (int i = 8; i < 12; i++)
			transfo3_MST->push_back((float)matrix.data()[i]);
		for (int i = 12; i < 16; i++)
			transfo4_MST->push_back((float)matrix.data()[i]);
	} //end-for-mst-edges
...
void Viewer::initialize_buffers() {
	rendering_program.bind();
	{
		...
		vao[29].bind();
		buffers[33].bind();
		buffers[33].allocate(pos_MST->data(), pos_MST->size() * sizeof(float));
		poly_vertexLocation[0] = rendering_program.attributeLocation("vertex");
		rendering_program.enableAttributeArray(poly_vertexLocation[0]);
		rendering_program.setAttributeBuffer(poly_vertexLocation[0], GL_FLOAT, 0, 3);
		buffers[33].release();
	...
	rendering_program_cylinders.bind();
	{
		...
		buffers[34].bind();
		buffers[34].allocate(transfo1_MST->data(), transfo1_MST->size() * sizeof(float));
		centerLocation[1] = rendering_program_cylinders.attributeLocation("transfo1");
		rendering_program_cylinders.enableAttributeArray(centerLocation[1]);
		rendering_program_cylinders.setAttributeBuffer(centerLocation[1], GL_FLOAT, 0, 4);
		buffers[34].release();
		...
		buffers[35].bind();
		buffers[35].allocate(transfo2_MST->data(), transfo2_MST->size() * sizeof(float));
		centerLocation[2] = rendering_program_cylinders.attributeLocation("transfo2");
		rendering_program_cylinders.enableAttributeArray(centerLocation[2]);
		rendering_program_cylinders.setAttributeBuffer(centerLocation[2], GL_FLOAT, 0, 4);
		buffers[35].release();
		...
		buffers[36].bind();
		buffers[36].allocate(transfo3_MST->data(), transfo3_MST->size() * sizeof(float));
		centerLocation[3] = rendering_program_cylinders.attributeLocation("transfo3");
		rendering_program_cylinders.enableAttributeArray(centerLocation[3]);
		rendering_program_cylinders.setAttributeBuffer(centerLocation[3], GL_FLOAT, 0, 4);
		buffers[36].release();
		...
		buffers[37].bind();
		buffers[37].allocate(transfo4_MST->data(), transfo4_MST->size() * sizeof(float));
		centerLocation[4] = rendering_program_cylinders.attributeLocation("transfo4");
		rendering_program_cylinders.enableAttributeArray(centerLocation[4]);
		rendering_program_cylinders.setAttributeBuffer(centerLocation[4], GL_FLOAT, 0, 4);
		buffers[37].release();
...
void Viewer::draw() {
	...
	if (m_isFlat || !extension_is_found) {
		...
		if (m_showMST){
			...
			vao[29].bind();
			rendering_program.setUniformValue(colorLocation[0], m_colorDEdge);
			glDrawArrays(GL_LINES, 0, pos_MST->size() / 3);
			vao[29].release();
			rendering_program.release();
		...
		if (m_showMST) {
			...
			vao[29].bind();
			rendering_program_cylinders.setUniformValue(colorLocation[2], m_colorDEdge);
			glDrawArraysInstanced(GL_TRIANGLES, 0, points_cylinder->size() / 3, transfo1_MST->size() / 4);
			vao[29].release();
			rendering_program_cylinders.release();
		}
...
void Viewer::mousePressEvent(QMouseEvent *event) {
	...
	if (m_curMode == INSERT_V && event->button() == Qt::LeftButton && modifiers == Qt::SHIFT) {
		m_isPress = true;
	} //end-if-insv
	...
	else if (m_curMode == FINDNB && event->button() == Qt::LeftButton && modifiers == Qt::SHIFT) {
		if (m_pScene->m_dt.is_valid() && m_pScene->m_dt.dimension() == 3)
			m_isPress = true;
		else
			displayMessage(tr("There exists no triangulation yet."));
	...
	else if (m_curMode == EMPTYSPH && event->button() == Qt::LeftButton && modifiers == Qt::SHIFT) {
		if (m_pScene->m_dt.is_valid() && m_pScene->m_dt.dimension() == 3)
			m_isPress = true;
		else
			displayMessage(tr("There exists no triangulation yet."));
...
void Viewer::wheelEvent(QWheelEvent *event) {
	...
	if ((m_curMode == INSERT_V || m_curMode == FINDNB || m_curMode == EMPTYSPH) && modifiers == Qt::SHIFT) {
...
void Viewer::keyPressEvent(QKeyEvent *event) {
	...
	if (m_curMode == INSERT_PT && m_hasNewPt && (event->key() == Qt::Key_Return || event->key() == Qt::Key_Enter) && modifiers == Qt::NoButton) {
	...
	else if (m_curMode == INSERT_PT && m_hasNewPt && event->key() == Qt::Key_Escape && modifiers == Qt::NoButton) {
		m_hasNewPt = false;
		// erase old conflict hole info
		m_boundaryFacets.clear();
		m_conflictCells.clear();
	...
	else if (m_curMode == SELECT && event->key() == Qt::Key_Delete && modifiers == Qt::NoButton) {
	...
	else if (m_curMode == SELECT && event->key() == Qt::Key_Escape && modifiers == Qt::NoButton) {
		// clear the selection buffer
		for (QList<int>::iterator iit = m_vidSeled.begin(); iit < m_vidSeled.end(); ++iit) {
			m_pScene->m_vhArray.at(*iit)->setSeled(false);
		}
	...
	else if (m_curMode == EMPTYSPH && event->key() == Qt::Key_S && modifiers == Qt::NoButton) {
